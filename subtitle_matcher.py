...
class SubtitleMatcher:
    def __init__(
        self, video_ext: str, folder: str, suffix: Optional[str] = None
    ) -> None:
    ...
    def match(self):
        ...
        print(
            tabulate.tabulate(
                line_list, headers=["From", "To"], tablefmt="fancy_grid"
            )
        )
    ...
    def _get_sorted_file_list(self):
        ...
        subtitles = [
            it for it in items if it.endswith(".smi") or it.endswith(".srt")
        ]
    ...
    def _get_new_name(
        self, src: str, subtitle_ext: Literal["smi", "srt"]
    ) -> str:
        if self.suffix:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--folder", help="folder containing movies", default="./"
    )
    parser.add_argument(
        "-v",
        "--video_ext",
