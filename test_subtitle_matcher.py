...
def mocked_listdir(mocker: MockerFixture):
	video_src_list = [
	    "hello_2people.mp4", "hello_11people.mp4", "hello_1people.mp4"
	]
...
@pytest.mark.parametrize("sm", [SubtitleMatcher(VIDEO_EXT, PATH_ROOT, None), SubtitleMatcher(VIDEO_EXT, PATH_ROOT, LANG_SUFFIX)])
...
def test_get_sorted_file_list(sm: SubtitleMatcher, mocked_listdir: MagicMock):
	...
	assert actual_vids == list(
	    f"{PATH_ROOT}{it}" for it in
	    ["hello_1people.mp4", "hello_2people.mp4", "hello_11people.mp4"])
...
def test_match(sm: SubtitleMatcher, mocked_rename: MagicMock,
               mocked_listdir: MagicMock):
	sm.match()
...
def test_match_with_insufficient_subtitles(sm: SubtitleMatcher,
